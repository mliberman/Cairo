...
#ifndef USE_CAIRO_READ_FUNC_LEN_T
...
#endif
{
   ...
   while (!eof)
   {
      ...
      else if (rlen < CAIRO_JPEG_IO_BLOCK_SIZE)
      {
         ++eof;
      }
      ...
      // read bytes into buffer and check for error
      status = read_func(closure, data + len, CAIRO_JPEG_IO_BLOCK_SIZE);

      // assume any unsuccessful read is EOF
